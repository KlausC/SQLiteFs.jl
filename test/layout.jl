...
@testset "simple layout templates" begin
    ...
    @test_throws ArgumentError is_template_fixed(Vector{Int})
    @test is_template_fixed(Float64)
    @test is_template_fixed(L1)
    @test is_template_fixed(Ptr{L1})
    @test is_template_fixed(Ptr{L5})
    @test is_template_fixed(LFixedVector{Int,10})
    @test_throws ArgumentError is_template_fixed(LFixedVector{Int})
    @test is_template_variable(LVarVector{Int,1})
    @test is_template_fixed(L2)
    ...
    @test_throws ArgumentError !is_template_fixed(LFixedVector{L5,2})
